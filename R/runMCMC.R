...
proposeParams <- function(param, fixedParams, S, nbStates) {
  if (length(param) == 3 * nbStates) {
    ...
    param[seq(2 * nbStates, length(param))[seq(2 * nbStates, length(param)) %% 3 != 0]] <- suppressWarnings(log(param[seq(2 * nbStates, length(param))[seq(2 * nbStates, length(param)) %% 3 != 0]]))
  }
  ...
  if (length(param) == 3 * nbStates) {
    ...
    thetasprime[intersect(seq(2 * nbStates, length(param))[seq(2 * nbStates, length(param)) %% 3 != 0], which(is.na(unlist(fixedParams))))] <- exp(thetas[intersect(seq(2 * nbStates, length(param))[seq(2 * nbStates, length(param)) %% 3 != 0], which(is.na(unlist(fixedParams))))])
  }
