...
NumericVector dexp2(NumericVector x, Nullable<NumericVector> Exp)
{
...
// Sinc
// sinc function
NumericVector sinc(NumericVector x, Nullable<NumericVector> SIN)
{
  NumericVector res(x.size());
  for (int i = 0; i < x.size(); i++)
  {
...
// Sinch
// hyperbolic sinc
NumericVector sinch(NumericVector x, Nullable<NumericVector> SINH)
{
  NumericVector res(x.size());
  for (int i = 0; i < x.size(); i++)
  {
...
mat makeT(double tau_pos, double tau_vel, double dt)
{
  ...
  else if (!std::isinf(tau(0)) && tau(1) == 0)
  { // OU
    if (!std::isinf(dt))
    {
      dtau = dt / tau(0);
